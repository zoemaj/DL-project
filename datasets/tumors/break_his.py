...
class BHDataset(FewShotDataset, ABC):
    ...
    def load_break_his(self, mode='train', magnification=40, pretrained_model_name='conv_base', min_samples=20):
        path_embed = f'data/break_his/embeddings-with-{pretrained_model_name}/{mode}_data_{magnification}X.pt'
        path_labels = f'data/break_his/embeddings-with-{pretrained_model_name}/{mode}_labels_{magnification}X.pt'
        ...
        # Embeddings don't exist, create them
            ...
            data_tensor, labels_tensor = load_images(mode, file_name, magnification, pretrained_model_name)
            return data_tensor, labels_tensor
...
class BHSimpleDataset(BHDataset):
    def __init__(self, batch_size: int, root: str='./data/', mode: str='train', min_samples: int=20):
        ...
        self.pretrained_model_name = 'efficientnet_b0'
        self.initialize_data_dir(root, download_flag=False)
        self.samples, self.targets = self.load_break_his(mode, self.magnification, self.pretrained_model_name, min_samples)
...
class BHSetDataset(BHDataset):
    def __init__(self, n_way: int, n_support: int, n_query: int, n_episode: int=100, root: str='./data', mode: str='train'):
        ...
        samples_all, targets_all = self.load_break_his(mode, self.magnification, self.pretrained_model_name, min_samples)
