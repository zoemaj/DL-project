...
from datasets.tumors.break_his import *
...
def initialize_dataset_model(cfg):
    ''' 
    sets up the training and validation datasets, data loaders, and the model based on the provided configuration.
    '''
    ...
    if cfg.method.type == "baseline":
        ...
        train_dataset = instantiate(cfg.dataset.set_cls, mode='train') #cfg is the config file
    else:
...
def run(cfg): 
    ''' 
    the main entry point. It prints the configuration, initializes datasets and the model, and either trains or tests the model based on the specified mode.
    with cfg as the config file drom hydra main and the arguments from the command line such as:
    if python run.py exp.name={exp_name} method=maml dataset=tabula_muris then:
